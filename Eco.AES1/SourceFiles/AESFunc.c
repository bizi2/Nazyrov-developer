...
void fnAddRoundKey(uint8_t _round, state_t* _state, uint8_t* _roundKey)
{
	...
	for (i = 0; i < 4; ++i)
	{
		for (j = 0; j < 4; ++j)
		{
			(*_state)[i][j] ^= _roundKey[(_round * Nb * 4) + (i * Nb) + j];
		}
	}
}
...
void fnSubBytes(state_t* _state)
{
	...
	for (i = 0; i < 4; ++i)
	{
		for (j = 0; j < 4; ++j)
		{
			(*_state)[j][i] = fnGetSBoxValue((*_state)[j][i]);
		}
	}
}
...
void fnSubBytesInv(state_t* _state)
{
	...
	for (i = 0; i < 4; ++i)
	{
		for (j = 0; j < 4; ++j)
		{
			(*_state)[j][i] = fnGetSBoxInvertValue((*_state)[j][i]);
		}
	}
}
...
void fnMixColumnsInv(state_t* _state)
{
	...
	for (i = 0; i < 4; ++i)
	{
		...
		(*_state)[i][0] = fnMultiply(a, 0x0e) ^ fnMultiply(b, 0x0b) ^ fnMultiply(c, 0x0d) ^ fnMultiply(d, 0x09);
		(*_state)[i][1] = fnMultiply(a, 0x09) ^ fnMultiply(b, 0x0e) ^ fnMultiply(c, 0x0b) ^ fnMultiply(d, 0x0d);
		(*_state)[i][2] = fnMultiply(a, 0x0d) ^ fnMultiply(b, 0x09) ^ fnMultiply(c, 0x0e) ^ fnMultiply(d, 0x0b);
		(*_state)[i][3] = fnMultiply(a, 0x0b) ^ fnMultiply(b, 0x0d) ^ fnMultiply(c, 0x09) ^ fnMultiply(d, 0x0e);
	}
}
...
void fnKeyExpansion(uint8_t* _roundKey, uint8_t* _key)
{
	...
	for (i = Nk; i < Nb * (Nr + 1); ++i)
	{
		{
			k = (i - 1) * 4;
			l_tempa[0] = _roundKey[k + 0];
			l_tempa[1] = _roundKey[k + 1];
			l_tempa[2] = _roundKey[k + 2];
			l_tempa[3] = _roundKey[k + 3];
		...
		if (i % Nk == 0)
		{
			...
			//RotWord()
			{
				const uint8_t l_u8tmp = l_tempa[0];
				l_tempa[0] = l_tempa[1];
				l_tempa[1] = l_tempa[2];
				l_tempa[2] = l_tempa[3];
				l_tempa[3] = l_u8tmp;
			...
			//Subword()
			{
				l_tempa[0] = fnGetSBoxValue(l_tempa[0]);
				l_tempa[1] = fnGetSBoxValue(l_tempa[1]);
				l_tempa[2] = fnGetSBoxValue(l_tempa[2]);
				l_tempa[3] = fnGetSBoxValue(l_tempa[3]);
			}

			l_tempa[0] = l_tempa[0] ^ g_Rcon[i / Nk];
		}
#if defined(AES256) && (AES256 == 1)
		if (i % Nk == 4)
		{
...
#endif
		j = i * 4; k = (i - Nk) * 4;
		_roundKey[j + 0] = _roundKey[k + 0] ^ l_tempa[0];
		_roundKey[j + 1] = _roundKey[k + 1] ^ l_tempa[1];
		_roundKey[j + 2] = _roundKey[k + 2] ^ l_tempa[2];
		_roundKey[j + 3] = _roundKey[k + 3] ^ l_tempa[3];
	}
}
